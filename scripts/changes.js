$('document').ready(function() {
  ...
  d3.csv('../data/dictionary.csv').then(function(rows) {
    ...
    // Changes by variable by town:
    // update Variables list on table change
    ...
    d3.csv('../data/data.csv').then(function(rows) {
      ...
      $('#variables').change(function() {
        ...
        // Update horizontal bar chart
        /*
        chart2.data.labels = towns;
        chart2.data.datasets[0].data = change;
        chart2.update(); */
        ...
        for (var i = 0; i < towns.length; i++) {
          var change = getChange(after[i], before[i]);
          datatable2.rows.add([ [towns[i], before[i], after[i], change ? change + '%' : ''] ])
        }
        datatable2.draw();
